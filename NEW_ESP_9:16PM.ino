    ...
    if(fb) {
      ...
      if(err == EI_IMPULSE_OK) {
        for(size_t i = 0; i < result.bounding_boxes_count; i++) {
          if(result.bounding_boxes[i].value > 0.92) {
...
for (size_t i = 0; i < length; i++) {
        // Compute x, y in the model-sized (e.g. 96x96) image
        size_t x = (offset + i) % EI_CLASSIFIER_INPUT_WIDTH;
        size_t y = (offset + i) / EI_CLASSIFIER_INPUT_WIDTH;

        // Map x, y to the original 320x240 image
        size_t xi = x * (EI_CAMERA_RAW_FRAME_BUFFER_COLS / EI_CLASSIFIER_INPUT_WIDTH);
        size_t yi = y * (EI_CAMERA_RAW_FRAME_BUFFER_ROWS / EI_CLASSIFIER_INPUT_HEIGHT);

        // Calculate index into the snapshot_buf
        size_t pixel_index = (yi * EI_CAMERA_RAW_FRAME_BUFFER_COLS + xi) * 3;

        // Read RGB values
        uint8_t r = snapshot_buf[pixel_index];
        uint8_t g = snapshot_buf[pixel_index + 1];
        uint8_t b = snapshot_buf[pixel_index + 2];

        // Convert to single float (RGB packed as R << 16 | G << 8 | B)
